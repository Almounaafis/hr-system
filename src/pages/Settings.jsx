...
export default function PayrollAttendancePolicy() {
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      const newDeductionPolicy = {
        ...
        discountFactor: settingsData.deduction_multiplier
          ? String(parseFloat(settingsData.deduction_multiplier))
          : '2',
  ...
  const handleCreateShift = async (shiftPayload) => {
    try {
      ...
      logger.error('Error creating/updating shift:', error);
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        {/* Title */}
        <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-foreground">إعدادات النظام</h1>
      ...
      {/* Tabs */}
      <TabBar activeTab={activeTab} onChange={setActiveTab} className="text-sm sm:text-base" />

      {/* Content */}
      <div className="text-sm sm:text-base">
        <FormProvider {...methods}>{renderActiveTab()}</FormProvider>
      </div>
